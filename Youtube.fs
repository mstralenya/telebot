...
let getYoutubeReply (url: string)=
    async {
        ...
        if video.Duration.Value > TimeSpan.FromMinutes(5.0) then
            printfn "The video is longer than 5 minutes. Download aborted."
            return None
        else
            let fileName = $"yt_{video.Id}_{Guid.NewGuid()}.mp4"
            printfn $"Downloading video: %s{video.Title}"
            ...
            let audioStream = streamManifest.GetAudioOnlyStreams()
             ...
             // Download the video
            printfn $"Downloading stream: %s{videoStream.Url}"
            ...
            printfn $"Video downloaded successfully: %s{fileName}"
